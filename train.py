...
def run_validation(
    ...
    with torch.no_grad():
        for batch in validation_ds:
            ...
            # Lấy câu gốc và câu dịch chuẩn
            source_text = batch['src_text'][0]
            target_text = batch['tgt_text'][0]

            # Chuyển token ID thành câu tiếng Ý
...
def get_ds(config):
    ...
    # Build tokenizers
    tokenizer_src = get_or_build_tokenizer(config, ds_raw, config['lang_src']) #tạo source
    tokenizer_tgt = get_or_build_tokenizer(config, ds_raw, config['lang_tgt']) #tạo target
...
def train_model(config):
    ...
    if torch.xpu.is_available():
        ...
        print("Using Intel GPU:", torch.xpu.get_device_name(0))
    else:
    ...
    loss_fn = nn.CrossEntropyLoss(ignore_index=tokenizer_tgt.token_to_id('[PAD]'),label_smoothing=0.0).to(device)

    for epoch in range(initial_epoch, config['num_epochs']):
        
        batch_iterator = tqdm(train_dataloader, desc=f'Processing epoch {epoch:02d}') # tạo thanh tiến trình
        for batch in batch_iterator: # lấy từng bacth
            model.train() # đưa model sang chế độ training
